...
func (l *list_repo) GetListWOSub(limit, offset int) (*database.Lists, error) {
	...
	if result.RowsAffected == 0 {
		return nil, errors.New("list datas not found")
	}
	if result.Error != nil {
		return nil, errors.New("failed obtain list data")
	}
	return &datas, nil
}
...
func (l *list_repo) GetListAllWOSub() (*database.Lists, error) {
	var datas database.Lists
...
func (l *list_repo) UpdateList(data *database.List, id int) (*database.List, error) {
	...
	if res.Error != nil {
		return nil, errors.New("failed update data list")
	}
...
func (l *list_repo) DeleteList(id int) error {
	...
	if result.RowsAffected == 0 {
		return errors.New("list data not found")
	}
	if result.Error != nil {
		return errors.New("failed obtain data list")
	}

	res := l.db.Where("list_id = ?", id).Delete(&data)
	if res.Error != nil {
		return errors.New("failed delete list data")
	}
